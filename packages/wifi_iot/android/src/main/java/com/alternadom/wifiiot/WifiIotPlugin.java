...
public class WifiIotPlugin
  ...
  private void connect(final MethodCall poCall, final Result poResult) {
    new Thread() {
      public void run() {
        ...
        connectTo(poResult, ssid, bssid, password, security, joinOnce, withInternet, isHidden, usePatternMatcher);
      }
    }.start();
  }
  ...
  private void _findAndConnect(final MethodCall poCall, final Result poResult) {
    new Thread() {
      public void run() {
        ...
        connectTo(poResult, ssid, bssid, password, security, joinOnce, withInternet, false, false);
      }
    }.start();
  }
  ...
  private void connectTo(
      ...
      final Boolean usePatternMatcher) {
    final Handler handler = new Handler(Looper.getMainLooper());
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
